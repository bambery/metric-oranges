    ...
    with open(file_path_fips_adjacency, encoding='cp1252', newline='') as file:
        ...
        for row in reader:
            if row[0]: # if there is an entry in col 0, then this is begins a new adjacency entry 
                ...
                # create edges for each airport
                for locid in my_fips.airports:
                    flight_edge = Edge(my_fips.uid, Airport.get_uid(locid), "FR") 
                    Edge.collection[my_fips.uid].add(flight_edge)
